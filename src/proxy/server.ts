...
function resolveTargetModel(rule, group, requestBody) {
  ...
  const mappings =
    rule.modelMappings && typeof rule.modelMappings === "object" ? rule.modelMappings : {}
...
function toOpenAIResponsesUsage(anthropicUsage) {
  const inputTokens = Number.isFinite(anthropicUsage?.input_tokens)
    ? anthropicUsage.input_tokens
    : 0
  const outputTokens = Number.isFinite(anthropicUsage?.output_tokens)
...
class ProxyServer {
  ...
  async handleRequest(req, res, traceId) {
    ...
    try {
      ...
      if (config.server.authEnabled) {
        ...
        if (auth !== expected) {
          const payload = {
            error: { code: "unauthorized", message: "Missing or invalid local bearer token" },
          }
      ...
      if (method !== "POST" || !parsedPath) {
        const payload = {
          error: {
            code: "not_found",
            message: "Use POST /oc/:groupId/:endpoint (messages/chat/completions/responses)",
          },
        }
      ...
      chain.direction =
        entry.protocol === "openai" && rule.protocol === "anthropic"
          ? "oc"
          : entry.protocol === "anthropic" && rule.protocol === "openai"
            ? "co"
            : null
      ...
      const requestedModel =
        typeof requestBody?.model === "string" ? requestBody.model : rule.defaultModel
      ...
      chain.upstreamResponseHeaders = toRedactedHeaders(
        toPlainHeaders(upstreamResponse.headers),
        config
      )
      ...
      if (entry.protocol === "openai" && downstreamProtocol === "anthropic") {
        const chatBody = mapAnthropicToOpenAIResponse(upstreamJson, {
  ...
  async bridgeAnthropicToOpenAIResponses(upstreamResponse, res, traceId, requestModel) {
    ...
    const ensureToolItem = payload => {
      const contentIndex = Number.isInteger(payload?.index)
        ? payload.index
        : toolStatesByContentIndex.size
