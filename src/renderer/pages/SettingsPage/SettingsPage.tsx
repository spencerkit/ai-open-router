...
export const SettingsPage: React.FC = () => {
  ...
  const isServerDirty = Boolean(
    config && portText !== String(config.server.port)
  )
    ...
    async (action: Exclude<RemoteSyncAction, null>, force = false) => {
      ...
      try {
        ...
        if (action === "upload") {
          ...
          showToast(
            result.changed
              ? t("settings.remoteUploadSuccess")
              : t("settings.remoteUploadNoChange"),
            "success"
          )
        ...
        showToast(t("settings.remotePullSuccess", { count: result.importedGroupCount || 0 }), "success")
        setPendingRemoteConflict(null)
      } catch (error) {
